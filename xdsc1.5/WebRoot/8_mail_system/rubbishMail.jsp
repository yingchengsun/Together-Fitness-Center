...
<script type="text/javascript">
	function refresh(){
		window.location.href="<%=request.getContextPath()%>/8_mail_system/rubbishmail.action";
	}
  ...
  <body>
  <c:choose>
  	<c:when test="${rubbishmailempty=='1'}">
	...
	<legend style="font-size: 12px;">友情提示</legend>
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span>
		<table width="350px;">		
		<tr><td width="100px;" align="right"><img width="40px" height="40px" src="<%=request.getContextPath() %>/8_mail_system/pictures/warn.png"></td><td style="font-size: 16px;">垃圾邮件箱中没有邮件</td></tr>
		</table>
	</fieldset>
	</div>
  	</c:when>
    ...
    <form action="tagmail.action">
    	<table >
    		<tr>
    			...
    			<td><input type="submit" value="标记" onclick="window.alert('邮件标记成功')" name="submit"></td>
    		</tr>
    	</table>
    	<div style="
    	position: absolute;
    	...
    	">
    		<table cellspacing="1" id="mytable">
    			...
    			<td width="200"><%=from %></td>
    				...
    				<td><a class="LINK21" onclick="javascript:window.alert('邮件已经被移动到收件箱');" href="<%=request.getContextPath()%>/8_mail_system/rubbishremove.action?msgnum=<%=(Integer)msgnumList.get(i) %>" >移出</a></td>
    			</tr>
    			<%
    				}
    			 %>

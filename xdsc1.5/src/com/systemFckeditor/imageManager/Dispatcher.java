package com.systemFckeditor.imageManager;
			...
			try {
				...
				// Secure image check (can't be done if QuickUpload)
					...
					String newFileName = connector.fileUpload(type, context
							.getCurrentFolderStr(), newName, uplFile
							.getInputStream());			
...
//					String id=(String)request.getSession().getAttribute("Id");//得到当前图片的计数器
//					System.out.println("------------之前Id:"+id);
//					request.getSession().setAttribute("ImageName"+id, newName);
//					String ImageName=(String)request.getSession().getAttribute("ImageName"+id);
//					System.out.println("图片存放在session的key为:ImageName"+id+"存储名为："+ImageName);
//					int newId=Integer.parseInt(id)+1;
//					String pId=Integer.toString(newId);
//					request.getSession().setAttribute("Id", pId);
//					System.out.println("------------之后Id:"+pId);
